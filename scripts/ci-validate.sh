...
# block-k from the TP2 worker and issue191/async from the TP3 worker2; the
# rope-swa fix then landed on worker2 only, so rank 1 would boot stock YaRN
# while rank 0 booted patched -- mixed RoPE across ranks, with no error).
for fn in remote_compose remote_compose2; do
  ...
  if [ "$(grep -c "^$fn() {" start-deepseek-v4-flash-dspark.sh)" = 1 ] \
    ...
    && grep -Fq 'DSPARK_ENABLE_ROPE_SWA_FIX=$REMOTE_ROPE_SWA_FIX' <<<"$body"; then
    ok "$fn carries the full passthrough set exactly once"
  else
    bad "$fn must be defined exactly once and carry issue191/async/block-k/rope-swa passthroughs"
  fi
done
